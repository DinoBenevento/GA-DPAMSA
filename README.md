...
```
Then appropriately configure the [conf.py](./config.py) file according to your needs ([see next section for info](#configuration))

## Configuration
Several parameters can be set from the [conf.py](./config.py) file.
...
python3 mainGA.py
```
To change dataset for inference, you need to insert it into the [datasets/inference_dataset](./datasets/inference_dataset/) folder and import it into the mainGA.py file, essentially you need to modify the first line, indicating the name of the dataset.
From the mainGA.py file it is possible to change the type of mutation and the type of crossover of the genetic algorithm.
